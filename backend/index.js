  ...
  //define environment parameters (these will prompt during deployment)
  ...
  function extractCarInfo(message) {
    return extractCarInfoImproved(message);
  }

  async function handleParkingStatus(guestInfo, hotelHotel) {
    try {
  ...
  async function handleParkingEdit(userMessage, guestInfo, hotelHotel) {
  try {
    ...
    const conversationState = await getConversationState(guestInfo.phone, hotelHotel.id);
...
The updated pass is ready for pickup at the front desk.`;
  ...
  async function handleParkingRequest(userMessage, guestInfo, hotelHotel) {
    try {
      console.log(`Handling parking request for: ${guestInfo.isResident ? guestInfo.name : 'non-guest'}`);
      ...
      if (isTermRequest) {
        const greeting = guestInfo.isResident ? `Hi ${guestInfo.name}!` : "Hi there!";
      ...
      if (dateInfo.date && carInfo.licensePlate && carInfo.carMake) {
        return await createParkingPass(carInfo, dateInfo, guestInfo, hotelHotel);
      }
      ...
      const greeting = guestInfo.isResident ? guestInfo.name : "there";
  ...
  async function handleParkingFollowUp(userMessage, conversationHistory, guestInfo, hotelHotel) {
  // Get conversation state for this call
  const conversationState = await getConversationState(guestInfo.phone || guestInfo.phoneNumber, hotelHotel.id);
  return await handleParkingFollowUpImproved(userMessage, conversationHistory, guestInfo, hotelHotel, conversationState);
}
  ...
  async function createParkingPass(carInfo, dateInfo, guestInfo, hotelHotel) {
    try {
      ...
      const parkingData = {
        ...
        phoneNumber: guestInfo.isResident ? guestInfo.phone : guestInfo.phoneNumber,
        hotelId: hotelHotel.id,
...
async function getConversationState(phoneNumber, hotelId) {
    try {
      ...
      if (!stateSnapshot.empty) {
        ...
        // Create new state
        return await createNewConversationState(phoneNumber, hotelId);
      }
    } catch (error) {
  ...
  async function createNewConversationState(phoneNumber, hotelId) {
    try {
      const newState = {
        phoneNumber,
        hotelId,
  ...
  async function handleBookingStatus(userMessage, guestInfo, hotelHotel) {
    try {
      console.log(`Checking booking status for guest: ${guestInfo.id}`);
      
      // Get user's current serviceBookings from database
      const now = new Date();
      const serviceBookingsSnapshot = await db.collection('serviceBookings')
      ...
      console.log(`Found ${serviceBookingsSnapshot.docs.length} serviceBookings for guest`);
      
      if (serviceBookingsSnapshot.empty) {
        return `Hi ${guestInfo.name}! You don't have any current serviceBookings. Would you like to make a new booking?`;
      }
      
      const userBookings = serviceBookingsSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      
      // Filter to current and future serviceBookings
      const activeFutureBookings = userBookings.filter(booking => {
        const endDate = booking.endDate.toDate();
        return endDate >= now; // Include current and future serviceBookings
      });
      
      if (activeFutureBookings.length === 0) {
        return `Hi ${guestInfo.name}! You don't have any current or upcoming serviceBookings. Your previous serviceBookings may have ended. Would you like to make a new booking?`;
      }
      
      // Format the serviceBookings
      const serviceBookingsList = activeFutureBookings.map((booking, index) => {
      ...
      const pluralBookings = activeFutureBookings.length === 1 ? 'booking' : 'serviceBookings';
      ...
      console.error('Error details:', error.stack);
      return `Sorry ${guestInfo.name}, I'm having trouble checking your serviceBookings right now. Please try again or contact the front desk. Error: ${error.message}`;
    }
  }
  
  // Helper function to format serviceBookings list
  function formatBookingsList(serviceBookings, hotelHotel) {
    return serviceBookings.map((booking, index) => {
      const startDate = booking.startDate.toDate();
      const endDate = booking.endDate.toDate();
      ...
      const amenity = hotelHotel.amenities.find(a => a.id === booking.amenityId);
  ...
  exports.handleIncomingSMS = onRequest(
    ...
    async (req, res) => {
      try {
        ...
        // Find which property Hotel this Twilio number belongs to
        const hotelHotel = await getHotelHotelByPhone(twilioNumber);
        
        if (!hotelHotel) {
          console.error(`No property Hotel found for Twilio number: ${twilioNumber}`);
          res.status(404).send('Hotel not found');
          return;
        }
        
        // Store incoming message in database
        await storeMessage(cleanPhoneNumber, messageBody, 'incoming', MessageSid, hotelHotel.id);
        
        // Check if sender is a guest of this property
        const guestInfo = await checkResident(cleanPhoneNumber, hotelHotel.id);
        
        // Get conversation history
        const conversationHistory = await getConversationHistory(cleanPhoneNumber, hotelHotel.id);
        ...
        await storeMessage(cleanPhoneNumber, aiResponse, 'outgoing', null, hotelHotel.id);
  ...
  // Function to find property Hotel by phone number
  async function getHotelHotelByPhone(phoneNumber) {
    try {
      const companiesSnapshot = await db.collection('HotelCompanies')
        .where('HotelPhone', '==', phoneNumber)
        .limit(1)
        .get();
      
      if (!companiesSnapshot.empty) {
        const HotelDoc = companiesSnapshot.docs[0];
        return {
          id: HotelDoc.id,
          ...HotelDoc.data()
        };
      ...
      console.error('Error finding property Hotel:', error);
      return null;
    }
  }
  
  // Function to check if phone number belongs to a guest
  async function checkResident(phoneNumber, hotelId) {
    try {
      console.log(`Checking guest for phone: ${phoneNumber}, Hotel: ${hotelId}`);
  ...
  async function analyzeMessageWithImprovedContext(message, guestInfo, hotelHotel, conversationHistory = [], conversationState) {
    try {
      const lowerMessage = message.toLowerCase().trim();
        ...
        if (conversationState.context === 'booking') {
          ...
          if (conversationState.flowStep === 'awaiting_amenity') {
            ...
            if (numberMatch) {
              ...
              if (amenityIndex >= 0 && amenityIndex < hotelHotel.amenities.length) {
                return { isBookingRequest: true, flowContinuation: true };
              }
            }
            
            // Check for amenity name
            const amenity = hotelHotel.amenities.find(a => 
      ...
      const bookingStatusKeywords = [
        'my serviceBookings', 'my booking', 'show booking', 'list booking', 'current booking',
        'what booking', 'any booking', 'have booking', 'booking status', 'show my serviceBookings'
      ];
  ...
  async function handleGeneralMessage(userMessage, conversationHistory, guestInfo, hotelHotel, conversationState) {
    try {
      ...
      const systemPrompt = buildSystemPromptWithContext(guestInfo, hotelHotel, conversationContext);
  ...
  async function generateAIResponseWithImprovedState(userMessage, conversationHistory, guestInfo, hotelHotel) {
    try {
      console.log(`\n=== PROCESSING MESSAGE ===`);
      console.log(`Message: "${userMessage}"`);
      console.log(`Resident: ${guestInfo.isResident ? guestInfo.name : 'non-guest'}`);
      ...
      const analysis = await analyzeMessageWithImprovedContext(userMessage, guestInfo, hotelHotel, conversationHistory, conversationState);
      ...
      if (!analysis || typeof analysis !== 'object') {
        ...
        return await handleGeneralMessage(userMessage, conversationHistory, guestInfo, hotelHotel, conversationState);
      }
      ...
      if (analysis.isMaintenanceIssue) {
        ...
        return await handleMaintenanceIssue(userMessage, guestInfo, hotelHotel);
      }
      
      // Handle parking requests and status - ALWAYS AVAILABLE
      if (analysis.isParkingStatus && guestInfo.isResident) {
        console.log('Handling parking status request');
        return await handleParkingStatus(guestInfo, hotelHotel);
      }
      
      if (analysis.isParkingEdit && guestInfo.isResident) {
        console.log('Handling parking edit request');
        return await handleParkingEdit(userMessage, guestInfo, hotelHotel);
      }
      
      if (analysis.isParkingRequest) {
        console.log('Handling parking request');
        const response = await handleParkingRequest(userMessage, guestInfo, hotelHotel);
      ...
      if (analysis.isParkingFollowUp) {
        ...
        const response = await handleParkingFollowUpImproved(userMessage, conversationHistory, guestInfo, hotelHotel, conversationState);
      ...
      if (analysis.isBookingStatus && guestInfo.isResident) {
        console.log('Handling booking status request');
        return await handleBookingStatus(userMessage, guestInfo, hotelHotel);
      }
      
      if (analysis.isBookingManagement && guestInfo.isResident) {
        console.log('Handling booking management request');
        return await handleBookingManagement(userMessage, guestInfo, hotelHotel);
      }
      
      if (analysis.isBookingRequest) {
        console.log('Handling booking request');
        if (guestInfo.isResident) {
          const response = await handleBookingRequestWithState(userMessage, guestInfo, hotelHotel, conversationState);
          ...
          return "Hi there! I'd love to help, but amenity serviceBookings are only available for registered guests. Please visit the front desk in person or ask a guest with their phone number on file to make the booking for you.";
        }
      }
  ...
  async function handleMaintenanceIssue(message, guestInfo, hotelHotel) {
    try {
      console.log('Creating maintenance issue for:', guestInfo.isResident ? guestInfo.name : 'non-guest');
      ...
      const issueData = {
        ...
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        reportedBy: guestInfo.isResident ? 'guest' : 'visitor',
      ...
      const greeting = guestInfo.isResident ? guestInfo.name : "there";
      return `Sorry ${greeting}, I had trouble logging your maintenance request. Please call the front desk directly at ${hotelHotel.phone || 'the main number'}.`;
    }
  }
  
  async function handleBookingRequestWithState(userMessage, guestInfo, hotelHotel, conversationState) {
    try {
      ...
      if (!selectedAmenity) {
        for (const amenity of hotelHotel.amenities) {
      ...
      if (!selectedAmenity) {
        const amenityList = hotelHotel.amenities
      ...
      return await serviceBookingsWithAvailabilityCheck(selectedAmenity, startTime, endTime, guestInfo, hotelHotel, conversationState);
  ...
  async function serviceBookingsWithAvailabilityCheck(amenity, startTime, endTime, guestInfo, hotelHotel, conversationState) {
    try {
      ...
      if (!availabilityResult.available) {
        if (availabilityResult.reason === 'outside_business_hours') {
          ...
          return `Sorry ${guestInfo.name}, the ${amenity.name} is already booked during that time. Current serviceBookings: ${conflictTimes}. Would you like to try a different time?`;
        } else {
      ...
      try {
        ...
        const bookingRef = await db.collection('serviceBookings').add(bookingData);
      ...
      console.error('Error in serviceBookingsWithAvailabilityCheck:', error);
      await resetConversationState(conversationState.id);
  ...
  function buildSystemPromptWithContext(guestInfo, hotelHotel, conversationContext) {
    const basePrompt = `You are Claro, a friendly virtual concierge for ${hotelHotel.name}. You're helpful, conversational, and efficient.
    
  Hotel Information:
  - Hotel: ${hotelHotel.name}
  - Address: ${hotelHotel.address}
  - Available amenities: ${hotelHotel.amenities.map(a => a.name).join(', ')}
  ...
  - Help with amenity serviceBookings (guests only)
  ...
  RESIDENT: ${guestInfo.name} (Unit ${guestInfo.roomNumber})
  ...
  async function handleParkingFollowUpImproved(userMessage, conversationHistory, guestInfo, hotelHotel, conversationState) {
    try {
      ...
      if (finalDateInfo.date && finalCarInfo.licensePlate && finalCarInfo.carMake) {
        return await createParkingPass(finalCarInfo, finalDateInfo, guestInfo, hotelHotel);
      }
  ...
  async function getConversationHistory(phoneNumber, hotelId, limit = 10) {
    try {
  ...
  async function storeMessage(phoneNumber, content, direction, twilioMessageSid = null, hotelId) {
    try {
      const messageData = {
        phoneNumber,
        content,
      ...
      console.log(`Stored ${direction} message for ${phoneNumber} at Hotel ${hotelId}`);
    } catch (error) {
  ...
  async function analyzeBookingRequest(message, guestInfo, hotelHotel, conversationHistory = []) {
    try {
      ...
      const managementKeywords = ['edit', 'change', 'update', 'cancel', 'modify', 'reschedule', 'my booking', 'my serviceBookings', 'show my'];
      ...
      const hasRecentAmenityMention = recentMessages.some(msg => 
        msg.content && hotelHotel.amenities.some(a => 
      ...
      if (hasActiveBookingFlow && hasTimeInfo) {
        ...
        for (const msg of recentMessages.reverse()) {
          if (msg.content) {
            amenity = hotelHotel.amenities.find(a => 
              msg.content.toLowerCase().includes(a.name.toLowerCase()) || 
              msg.content.toLowerCase().includes(a.type.toLowerCase())
            );
            if (amenity) {
      ...
      // Check if we need end time for single time serviceBookings
      const needsEndTime = timeInfo.startTime && !timeInfo.endTime && !lowerMessage.includes('-');
  ...
  async function handleBookingUpdate(userMessage, guestInfo, hotelHotel) {
    try {
      ...
      // Filter to upcoming serviceBookings and sort by start date
      const upcomingBookings = userBookings
        .filter(booking => booking.startDate.toDate() > now)
        .sort((a, b) => a.startDate.toDate() - b.startDate.toDate());
      
      if (upcomingBookings.length === 0) {
        return `Hi ${guestInfo.name}! I don't see any upcoming serviceBookings to update. Would you like to make a new booking?`;
      }
      ...
      const amenity = hotelHotel.amenities.find(a => a.id === bookingToUpdate.amenityId);
      ...
      if (!availabilityResult.available) {
        if (availabilityResult.reason === 'outside_business_hours') {
          ...
          return `Sorry ${guestInfo.name}, the ${amenityName} is already booked during that time. Current serviceBookings: ${conflictTimes}. Would you like to try a different time?`;
        }
      }
  ...
  async function handleBookingManagementImproved(userMessage, guestInfo, hotelHotel) {
    try {
      const lowerMessage = userMessage.toLowerCase();
      
      console.log(`Handling booking management for guest: ${guestInfo.id}`);
      console.log(`Hotel: ${hotelHotel.id}`);
      ...
      // Filter to upcoming serviceBookings only
      const now = new Date();
      ...
      console.log(`Found ${upcomingBookings.length} upcoming serviceBookings`);
      
      if (upcomingBookings.length === 0) {
        return `Hi ${guestInfo.name}! You don't have any upcoming serviceBookings to manage. Would you like to make a new booking?`;
      }
      
      // Format serviceBookings list using the new helper function
      const serviceBookingsList = formatBookingsList(upcomingBookings, hotelHotel);
      ...
      // Default - show all serviceBookings
      ...
      console.error('Error details:', error.stack);
      return `Sorry ${guestInfo.name}, I'm having trouble accessing your serviceBookings right now. Please try again or contact the front desk. Error: ${error.message}`;
    }
  }
  
  // Function to handle availability checks
  async function handleAvailabilityCheck(userMessage, conversationHistory, guestInfo, hotelHotel) {
    try {
      const lowerMessage = userMessage.toLowerCase();
  ...
  async function handleBookingRequest(bookingRequest, guestInfo, hotelHotel) {
    try {
      console.log('Handling booking request:', bookingRequest);
      console.log('Resident info:', guestInfo);
      ...
      try {
        const bookingId = await serviceBookings(
          amenity.id,
          amenity.name,
          startTime,
          finalEndTime,
  ...
  async function checkAmenityAvailability(amenityId, startDate, endDate, hotelId) {
    try {
      console.log('Checking availability for:', {
        amenityId,
      ...
      // Get Hotel data for business hours if needed
      const HotelDoc = await db.collection('HotelCompanies').doc(hotelId).get();
      const HotelData = HotelDoc.data();
      
      // Check business hours if available
      if (HotelData.settings && HotelData.settings.businessHours) {
        const businessStart = HotelData.settings.businessHours.start; // "09:00"
        const businessEnd = HotelData.settings.businessHours.end; // "17:00"
      ...
      // Check for conflicting serviceBookings
      const serviceBookingsSnapshot = await db.collection('serviceBookings')
      ...
      console.log(`Found ${serviceBookingsSnapshot.docs.length} existing serviceBookings for amenity`);
      
      const conflictingBookings = serviceBookingsSnapshot.docs.filter(doc => {
      ...
      if (conflictingBookings.length > 0) {
        console.log(`Found ${conflictingBookings.length} conflicting serviceBookings`);
        return {
          available: false,
  ...
  async function serviceBookings(amenityId, amenityName, startDate, endDate, guestInfo, hotelId, notes = '') {
    try {
  ...
  async function updateBooking(bookingId, updateData) {
    try {
      ...
      await db.collection('serviceBookings').doc(bookingId).update(updatePayload);
  ...
  function buildSystemPrompt(guestInfo, hotelHotel) {
    const basePrompt = `You are Claro, a friendly and relaxed virtual concierge assistant for ${hotelHotel.name}. You're not a stern receptionist or a robotic agent — you're more like a chill, helpful front desk person who just wants to make things easy for the caller. You listen, empathize, and genuinely try to help or direct the person to the right team.
  ...
  - For amenity serviceBookings: ONLY guests can book amenities. Non-guests should be directed to visit the front desk or ask a registered guest to help them.
  ...
  - For amenity serviceBookings: Direct them to visit the front desk or ask a registered guest to help them
  ...
  exports.getAvailableSlots = onCall(
    ...
    async (request) => {
      try {
        const { amenityId, hotelId, date } = request.data;
        ...
        const bookedSlots = serviceBookingsSnapshot.docs.map(doc => {
          const booking = doc.data();
          return {
            start: booking.startDate.toDate(),
            end: booking.endDate.toDate()
  ...
  exports.getConversation = onCall(
    ...
    async (request) => {
      try {
        const { phoneNumber, hotelId } = request.data;
        ...
        const history = await getConversationHistory(phoneNumber, hotelId, 50);
  ...
  exports.sendManualMessage = onCall(
    ...
    async (request) => {
      try {
        ...
        const { phoneNumber, message, hotelId } = request.data;
        ...
        // Get Hotel phone number
        const Hotel = await db.collection('HotelCompanies').doc(hotelId).get();
        if (!Hotel.exists) {
          throw new Error('Hotel not found');
        }
        
        const HotelData = Hotel.data();
        await sendTwilioResponse(phoneNumber, message, HotelData.HotelPhone);
        await storeMessage(phoneNumber.replace(/^\+1/, ''), message, 'outgoing', null, hotelId);
